...
from telegram import (
    ...
    KeyboardButton, ReplyKeyboardMarkup, CopyTextButton,
)
...
def db_initial_load():
    """
    Restart bo'lganda (Fly.io / Railway / Render):
      - Lokal fayllar yo'qoladi — JSONBlob ASOSIY manba
      - JSONBlob yuklanmasa 3 marta qayta urinadi (30+60 sek kutib)
      - Hech narsa yuklanmasa — bot ishga tushmaydi (ma'lumot xavfsizligi)
    """
    logger.info("🔄 Ma'lumotlar yuklanmoqda (JSONBlob = asosiy manba)...")

    # ── 1. JSONBlob dan yuklash: yuklanmasa qayta urinish ──────
    blob = None
    if JSONBLOB_URL:
        for round_num in range(3):
            ...
            if round_num < 2:
                wait = 30 * (round_num + 1)
                logger.warning(
                    f"⏳ JSONBlob yuklanmadi ({round_num+1}/3), "
                    f"{wait} sek kutib qayta urinamiz...")
                time.sleep(wait)
    ...
    # ── 2. JSONBlob yuklanmadi ──────────────────────────────────
    if JSONBLOB_URL and blob is None:
        DB_STATUS.update({"storage_ok": False, "ram_only": True,
                          "last_err": "JSONBlob yuklanmadi", "load_failed": False})
        if has_local:
            # Lokal fayl bor (volume mount yoki lokal server) — undan yuklaymiz
            RAM.from_dict(local)
            EMOJI_IDS.clear()
            EMOJI_IDS.update(RAM.emoji_ids)
            logger.warning(
                f"⚠️ JSONBlob yuklanmadi! Lokal fayldan yuklanmoqda: "
                f"{len(RAM.movies)} kino, {len(RAM.users)} user. "
                "Periodik sync JSONBlob ga qayta urinadi.")
        else:
            # ❗ XAVFSIZLIK: na JSONBlob, na lokal — bo'sh RAM bilan ISHLAMAYMIZ!
            # Bo'sh RAM JSONBlob ga yozilsa barcha kinolar o'chib ketadi!
            logger.critical(
                "🚨 XAVFSIZLIK: JSONBlob ham, lokal fayl ham yuklanmadi! "
                "   Bot to'xtatilmoqda — bo'sh ma'lumot bilan ishlash xavfli.")
            raise RuntimeError(
                "JSONBlob dan yuklab bo'lmadi va lokal fayl ham yo'q. "
                "Bot ma'lumotlarni yo'qotmasligi uchun ishga tushmadi. "
                "JSONBlob URL ni tekshiring yoki biroz vaqt o'tib qayta ishga tushiring."
            )
        return

    # ── 3. Ikkala manba ham bo'sh — birinchi marta ishga tushish ──
    if not has_blob and not has_local:
        logger.warning("⚠️ Hech qanday ma'lumot topilmadi — birinchi marta ishga tushmoqda")
        RAM.loaded = True
        return

    # ── 4. Birlashtirish va RAMga yuklash ──────────────────────
    merged = _merge_db(blob, local)
    RAM.from_dict(merged)
    EMOJI_IDS.clear()
    EMOJI_IDS.update(RAM.emoji_ids)

    blob_movies  = blob.get("movies",  {}) if has_blob  else {}
    local_movies = local.get("movies", {}) if has_local else {}
    blob_eps   = sum(len(m.get("episodes", []) or []) for m in blob_movies.values())
    local_eps  = sum(len(m.get("episodes", []) or []) for m in local_movies.values())
    merged_eps = sum(len(m.get("episodes", []) or []) for m in RAM.movies.values())
    logger.info(
        f"✅ Birlashtirildi → RAM: {len(RAM.movies)} kino / {merged_eps} qism / "
        f"{len(RAM.users)} user | "
        f"   JSONBlob: {len(blob_movies)} kino / {blob_eps} qism | "
        f"Lokal: {len(local_movies)} kino / {local_eps} qism"
    )
...
def _ensure_lock():
    ...
    try:
        ...
        loop = None
    if _jsonblob_save_lock is None and loop is not None:
        _jsonblob_save_lock = asyncio.Lock()
    if _jsonblob_save_lock is None:
        # Fallback: yaratamiz, keyin loop ulangan vaqtda to'g'ri ishlaydi
        _jsonblob_save_lock = asyncio.Lock()
...
# ❗ Foydalanuvchi so'rovi debounce — bir xil tugma ko'p marta bosilmasin
_USER_LAST_MSG: dict[int, float] = {}
USER_MSG_DEBOUNCE = 1.5  # soniya


def _is_user_spam(uid: int) -> bool:
    """True bo'lsa — bu foydalanuvchi 1.5 soniya ichida yana xabar yubordi."""
    now = time.time()
    last = _USER_LAST_MSG.get(uid, 0)
    if now - last < USER_MSG_DEBOUNCE:
        return True
...
def ibtn(text, data=None, url=None, style=None, emoji_id=None):
    """InlineKeyboardButton yaratadi. style va emoji_id — vizual, PTB da e'tiborga olinmaydi."""
    kw = {"text": str(text)}
    if data:     kw["callback_data"] = str(data)[:64]
    if url:      kw["url"] = url
    if emoji_id: kw["icon_custom_emoji_id"] = str(emoji_id)
    return InlineKeyboardButton(**kw)

def rbtn(text, style=None, emoji_id=None):
    """KeyboardButton yaratadi."""
    return KeyboardButton(text=str(text))

def ikb(rows):
    """
    InlineKeyboardMarkup qaytaradi.
    rows — list of list of InlineKeyboardButton yoki dict (CopyTextButton uchun).
    """
    processed = []
    for row in rows:
        new_row = []
        for btn in row:
            if isinstance(btn, InlineKeyboardButton):
                ...
                # CopyTextButton yoki boshqa maxsus tugma
                copy_text_raw = btn.get("copy_text")
                if copy_text_raw:
                    copy_val = (copy_text_raw.get("text", "") if isinstance(copy_text_raw, dict)
                                else str(copy_text_raw))
                    new_row.append(InlineKeyboardButton(
                        text=str(btn.get("text", "Nusxa")),
                        copy_text=CopyTextButton(text=copy_val),
                    ))
                elif btn.get("callback_data") or btn.get("url"):
                    kw2 = {"text": str(btn.get("text", ""))}
                    if btn.get("callback_data"): kw2["callback_data"] = str(btn["callback_data"])[:64]
                    if btn.get("url"):           kw2["url"] = btn["url"]
                    new_row.append(InlineKeyboardButton(**kw2))
        if new_row:
            processed.append(new_row)
    return InlineKeyboardMarkup(processed)
    ...
    for row in rows:
        ...
        for btn in row:
            if isinstance(btn, KeyboardButton):
                ...
                new_row.append(KeyboardButton(text=str(btn.get("text", ""))))
            else:
                new_row.append(KeyboardButton(text=str(btn)))
        if new_row:
            processed.append(new_row)
    return ReplyKeyboardMarkup(processed, resize_keyboard=resize)
...
def payment_sent_kb(card: str = "", price: int = 0):
    ...
    if card:
        copy_row.append(InlineKeyboardButton(
            text=str(bt("karta_nusxa")),
            copy_text=CopyTextButton(text=str(card)),
        ))
    if price:
        copy_row.append(InlineKeyboardButton(
            text=str(bt("miqdor_nusxa")),
            copy_text=CopyTextButton(text=str(price)),
        ))
    if copy_row:
...
def generate_movies_image(movie_slice: list, page: int = 1, total_pages: int = 1,
    ...
    for idx, (code, movie) in enumerate(movie_slice):
        ...
        views_total = sum(movie.get("views", {}).values()) if movie.get("views") else 0
        sub_y = y0 + 100*SCALE
...
async def _send_kino_list_page(bot, chat_id: int, page: int = 0, query=None):
    ...
    # Barcha kinolar ko'rsatiladi (qismsiz ham)
    all_items = sorted(
        [(c, m) for c, m in movies.items() if isinstance(m, dict)],
    ...
    if not all_items:
        if query is not None:
            try:
                await query.answer("🎬 Hozircha hech qanday kino yo'q.", show_alert=True)
                return
            except Exception:
                pass
        await bot.send_message(chat_id,
            "🎬 <b>Hozircha hech qanday kino yo'q.</b>", parse_mode="HTML")
        return
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Spam himoya — faqat kino ro'yxati uchun ──
    if data.startswith("kino_list|") and _is_user_spam(uid):
        return
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if _main_btn("barcha_kino"):
        # Spam himoya — foydalanuvchi bir necha marta bossa
        if _is_user_spam(uid):
            return
        movies = RAM.movies
        if not movies:
            await sm(context.bot, uid,
...
async def admin_state_handler(update, context, text: str) -> bool:
    ...
    if state == "add_ep_video":
        # Video kutilayapti — oddiy matn kelsa jim o'tkazib yuboramiz
        # Lekin awaiting_check yoki awaiting_help holatlarini to'sib qo'ymaymiz
...
def main():
    ...
    async def _startup_notify(context_job):
        try:
            now_str = datetime.now().strftime("%H:%M:%S")
            # Ishga tushganda JSONBlob ga darhol saqlaymiz (restart xavfsizligi)
            ok = await asyncio.to_thread(_save_jsonblob, RAM.to_dict())
            if ok:
                DB_STATUS.update({"storage_ok": True, "last_save_ok": now_str,
                                  "ram_only": False, "fail_count": 0})
                storage_msg = f"🟢 JSONBlob — saqlandi ({now_str})"
            else:
                DB_STATUS.update({"storage_ok": False, "ram_only": True, "last_err": now_str})
                storage_msg = f"🔴 JSONBlob ishlamayapti! RAM only — xatarli!"
            ram_only_warn = (
                "\n\n⚠️ <b>DIQQAT:</b> JSONBlob ishlamayapti!\n"
                "Restart bo'lsa ma'lumotlar yo'qolishi mumkin!\n"
                "JSONBlob URL ni zudlik bilan tekshiring."
            ) if not ok else ""
            await context_job.bot.send_message(
                ADMIN_ID,
    ...
    if app.job_queue:
        ...
        # ── JSONBlob yuklanmagan bo'lsa — tezroq qayta urinish ──
        async def _retry_jsonblob_if_down(ctx):
            if not DB_STATUS.get("storage_ok") and len(RAM.movies) > 0:
                logger.info("🔁 JSONBlob down — qayta urinilmoqda...")
                ok = await asyncio.to_thread(_save_jsonblob, RAM.to_dict())
                now_str = datetime.now().strftime("%H:%M:%S")
                if ok:
                    DB_STATUS.update({"storage_ok": True, "ram_only": False,
                                      "fail_count": 0, "last_save_ok": now_str})
                    _save_local(RAM.to_dict())
                    try:
                        await ctx.bot.send_message(
                            ADMIN_ID,
                            f"✅ <b>JSONBlob tiklandi!</b> {now_str}\n"
                            f"RAMdagi {len(RAM.movies)} kino saqlandi.",
                            parse_mode="HTML")
                    except: pass
                    logger.info(f"✅ JSONBlob qayta tiklandi: {len(RAM.movies)} kino saqlandi")
        app.job_queue.run_repeating(_retry_jsonblob_if_down, interval=30, first=20)
        logger.info("🔄 Periodik sync yoqildi (har 2 daqiqada + JSONBlob retry har 30 sek)")
