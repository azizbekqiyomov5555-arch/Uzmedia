...
JSONBLOB_URL      = os.environ.get("JSONBLOB_URL", "")
...
class RamCache:
    ...
    def ensure_user(self, uid):
        ...
        with self._lock:
            ...
            if "balance_updated_at" not in u:
                u["balance_updated_at"] = 0
...
def _safe_ts_value(value) -> float:
    try:
        if value in (None, ""):
            return 0.0
...
def _merge_user_records(blob_users: dict, local_users: dict) -> dict:
    """Foydalanuvchilarni xavfsiz birlashtiradi.

    Oldingi kod lokal backupni to'liq ustun qo'yar edi. Hosting qayta ishga
    tushganda lokal backup eski bo'lsa, JSONBlobdagi yangi balans 100 so'm
    lokal 0 so'm bilan bosilib ketishi mumkin edi. Shu sabab balans
    "bir bor, bir yo'q" ko'rinardi.
    """
    merged = {}
    all_uids = set((blob_users or {}).keys()) | set((local_users or {}).keys())
    for uid in all_uids:
        b = (blob_users or {}).get(uid) or {}
        l = (local_users or {}).get(uid) or {}
        ...
        paid = {}
        paid.update(b.get("paid_episodes") if isinstance(b.get("paid_episodes"), dict) else {})
        paid.update(l.get("paid_episodes") if isinstance(l.get("paid_episodes"), dict) else {})
        item["paid_episodes"] = paid

        watched = {}
        watched.update(b.get("watched") if isinstance(b.get("watched"), dict) else {})
        watched.update(l.get("watched") if isinstance(l.get("watched"), dict) else {})
        item["watched"] = watched

        b_bal = _safe_int_value(b.get("balance"), 0)
        l_bal = _safe_int_value(l.get("balance"), 0)
        b_ts = _safe_ts_value(b.get("balance_updated_at"))
        l_ts = _safe_ts_value(l.get("balance_updated_at"))

        if b_ts or l_ts:
            item["balance"] = b_bal if b_ts > l_ts else l_bal
            item["balance_updated_at"] = max(b_ts, l_ts)
        else:
            # Eski bazalarda timestamp yo'q: ijobiy balansni 0 bilan bosib yubormaymiz.
            item["balance"] = max(b_bal, l_bal)
            item["balance_updated_at"] = 0

        item["topup_total"] = max(_safe_int_value(b.get("topup_total"), 0),
                                   _safe_int_value(l.get("topup_total"), 0))
        item["premium_until"] = max(_safe_ts_value(b.get("premium_until")),
                                     _safe_ts_value(l.get("premium_until")))
        merged[str(uid)] = item
    return merged
...
def _merge_db(blob: dict | None, local: dict | None) -> dict:
    ...
    # Foydalanuvchilar — balans yo'qolmasligi uchun maydonma-maydon birlashadi
    merged_users = _merge_user_records(
        (blob.get("users")  or {}) if isinstance(blob.get("users"),  dict) else {},
        (local.get("users") or {}) if isinstance(local.get("users"), dict) else {},
    )
...
def clear_admin_state(context):
    for key in [
        ...
        "topup_amount_flow", "pending_topup", "awaiting_topup_check",
    ]:
...
async def cb_episode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if price > 0 and not is_episode_paid(user_id, code, ep):
        ...
        if balance >= price:
            ...
            u_data["balance_updated_at"] = time.time()
...
def _start_topup_amount_flow(context):
    """Hisob to'ldirish miqdorini alohida flow sifatida saqlaydi.
    Bu admin_state bilan aralashib ketmasligi uchun kerak: aks holda raqamli
    miqdor kino kodi deb qidirilib, kino chiqarib yuborishi mumkin.
    """
    context.user_data.pop("pending_topup", None)
    context.user_data.pop("awaiting_topup_check", None)
    context.user_data.pop("awaiting_check", None)
    context.user_data.pop("pending_check", None)
    context.user_data.pop("pending_code", None)
    context.user_data.pop("admin_state", None)
    context.user_data["topup_amount_flow"] = True


def _clear_topup_amount_flow(context):
    context.user_data.pop("topup_amount_flow", None)
    if context.user_data.get("admin_state") == "topup_amount":
        context.user_data.pop("admin_state", None)


async def _handle_topup_amount_text(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str) -> bool:
    """Miqdor kiritish bosqichini eng avval ushlab qoladi.
    True qaytsa, xabar to'liq ishlangan bo'ladi va kino qidiruviga o'tmaydi.
    """
    if not (context.user_data.get("topup_amount_flow") or context.user_data.get("admin_state") == "topup_amount"):
        return False
    ...
    amount = int(clean)
    _clear_topup_amount_flow(context)
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── 0. Hisobni to'ldirish miqdori ───────────────────
    # Bu blok kino qidiruvidan ham, admin_state handleridan ham OLDIN turadi.
    # Shunda 100/1000 kabi miqdorlar kino kodi sifatida ishlanmaydi.
    if await _handle_topup_amount_text(update, context, text):
        return
