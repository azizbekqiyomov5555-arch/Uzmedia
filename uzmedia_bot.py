...
# ─── KONFIGURATSIYA ────────────────────────────────────────
BOT_TOKEN  = os.environ.get("BOT_TOKEN")  or "8774359442:AAF42zPCxWCP2imr3ugqpvKdYXl-Apw0qcc"
...
class RamCache:
    ...
    def __init__(self):
        ...
        self.channels: list = []
        self.card_number: str = "5614684700269911"
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("admin_state") == "topup_amount":
        ...
        # CheckCard orqali to'lov yaratamiz
        import random, string
        order_id2 = ''.join(random.choices(string.ascii_letters + string.digits, k=10))
        # Avval pending to'lovni bekor qilamiz (xuddi shu miqdorda)
        for pid_old, pay_old in list(RAM.pending_payments.items()):
            if (pay_old.get("type") == "topup_checkcard"
                    and pay_old.get("user_id") == uid
        ...
        if result.get("status") != "success":
            msg_err = result.get("message", "Noma'lum xato")
            await sm(context.bot, uid,
                f"❌ CheckCard xatosi: <b>{msg_err}</b>\n\n"
        ...
        copy_row = []
        if card and card != "Admin karta raqamini o'rnatmagan":
            copy_row.append({"text": "📋 Karta nusxalash", "copy_text": {"text": str(card)}})
        copy_row.append({"text": "📋 Miqdor nusxalash", "copy_text": {"text": str(amount)}})
        kb_rows = []
        if copy_row:
            kb_rows.append(copy_row)
        kb_rows.append([InlineKeyboardButton("🔄 Tekshirish", callback_data=f"cc_check|{pid}")])
        kb_rows.append([InlineKeyboardButton("❌ Bekor qilish", callback_data=f"cc_cancel|{pid}")])
        topup_auto_kb = InlineKeyboardMarkup(kb_rows)

        card_line = f"💳 Karta: <code>{card}</code>\n" if card and card != "Admin karta raqamini o'rnatmagan" else ""
        await sm(context.bot, uid,
