...
# ─── KONFIGURATSIYA ────────────────────────────────────────
BOT_TOKEN  = "8774359442:AAGHUh69s8gTfyq4euK8_ZprSxjJFG4ELUg"
ADMIN_ID   = 7812447850

JSONBLOB_URL        = "https://jsonblob.com/api/jsonBlob/019e28c9-0d70-7fbf-9b65-231d4a399811"
JSONBLOB_BACKUP_URL = "https://jsonblob.com/api/jsonBlob/019df4aa-10b1-725c-83f5-9901ab2db9b6"  # Eski blob — birinchi yuklanishda ko'chiriladi
GSHEET_ID         = "1Co2okc7hgiWyJQvMnEx8ZSoGWuyqjwCi8CPrE_B5Lc4"

# ─── CHECKCARD API KONFIGURATSIYASI ─────────────────────────
CHECKCARD_SHOP_ID  = "924672"
CHECKCARD_SHOP_KEY = "DSSD85MU60"
CHECKCARD_BASE_URL = "https://checkcard.uz/api"

# ─── RAILWAY / WEBHOOK KONFIGURATSIYASI ─────────────────────
RAILWAY_URL            = "https://uzmedia-production.up.railway.app"
CHECKCARD_WEBHOOK_PATH = "/checkcard_webhook"
GSHEET_API             = ""
NPOINT_URL             = ""

# ─── FIREBASE KONFIGURATSIYASI ──────────────────────────────
FIREBASE_PROJECT_ID  = "uzmediyabit"
FIREBASE_DB_URL      = "https://uzmediyabit-default-rtdb.firebaseio.com"
FIREBASE_DB_PATH     = "bot_data"
FIREBASE_CREDS_FILE  = "uzmediyabit-firebase-adminsdk-fbsvc-6f2ff3f7fb.json"
FIREBASE_CREDS_JSON  = ""
...
def _fetch_one_blob(url: str) -> dict | None:
    """Bitta JSONBlob URL dan yuklash — 3 marta urinadi."""
    last_err = None
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if isinstance(data, dict):
                    logger.info(f"✅ Blob yuklandi ({url[-20:]}): "
                                f"{len(data.get('movies', {}))} kino, "
                                f"{len(data.get('users', {}))} user")
                    return data
                else:
                    logger.error(f"Blob noto'g'ri format: {type(data)}")
            else:
                logger.error(f"Blob load #{attempt+1} status {r.status_code} | {url[-20:]}")
        except Exception as e:
            last_err = e
            logger.error(f"Blob load #{attempt+1} xato: {e}")
        if attempt < 2:
            time.sleep(2 * (attempt + 1))
    logger.error(f"❌ Blob yuklanmadi: {url[-30:]} | {last_err}")
    return None


def _load_jsonblob() -> dict | None:
    """Asosiy + backup JSONBlob dan yuklash, birlashtirish."""
    if not JSONBLOB_URL:
        return None

    main_data   = _fetch_one_blob(JSONBLOB_URL)
    backup_data = None

    # Backup blob — faqat asosiy bo'sh bo'lsa YOKI kinolar 0 ta bo'lsa
    backup_url = globals().get("JSONBLOB_BACKUP_URL", "")
    if backup_url and backup_url != JSONBLOB_URL:
        main_movies = (main_data or {}).get("movies", {})
        if not main_movies:
            logger.info("ℹ️ Asosiy blob bo'sh — backup blob tekshirilmoqda...")
            backup_data = _fetch_one_blob(backup_url)

    if not main_data and not backup_data:
        return None

    # Birlashtirish — backup ustun (eski kinolar saqlansin)
    if backup_data and main_data:
        merged_movies = {**backup_data.get("movies", {}), **main_data.get("movies", {})}
        merged_users  = {**backup_data.get("users",  {}), **main_data.get("users",  {})}
        result = {**backup_data, **main_data}
        result["movies"] = merged_movies
        result["users"]  = merged_users
        logger.info(f"🔗 Asosiy+backup birlashtirildi: {len(merged_movies)} kino")
        # Birlashtirilganni asosiy blobga yozib qo'yamiz
        try:
            threading.Thread(
                target=lambda: requests.put(
                    JSONBLOB_URL,
                    headers={"Content-Type": "application/json", "Accept": "application/json"},
                    data=json.dumps(result), timeout=60
                ), daemon=True
            ).start()
            logger.info("⏳ Birlashtirilgan ma'lumot asosiy blobga sync qilinmoqda...")
        except Exception as e:
            logger.warning(f"Birlashtirilgan ma'lumot yozilmadi: {e}")
        return result
    elif backup_data:
        logger.info(f"📦 Faqat backup blob ishlatildi: {len(backup_data.get('movies', {}))} kino")
        return backup_data
    return main_data
...
def db_initial_load():
    ...
    # ── CARD_NUMBER — bazada bo'lmasa, koddan yuklaymiz ──
    if not RAM.card_number and CARD_NUMBER:
        RAM.card_number = CARD_NUMBER
        logger.info(f"💳 Karta raqami koddan yuklandi: {CARD_NUMBER}")
...
def payment_sent_kb(card: str = "", price: int = 0):
    ...
    copy_row = []
    if card:
        copy_row.append(InlineKeyboardButton(text=bt("karta_nusxa"), copy_text=str(card)))
    if price:
        copy_row.append(InlineKeyboardButton(text=bt("miqdor_nusxa"), copy_text=str(price)))
    if copy_row:
...
def generate_movies_image(movie_slice: list, page: int = 1, total_pages: int = 1,
    ...
    # Font izlash — avval loyiha papkasidan, keyin system fontlar
    _bot_dir = os.path.dirname(os.path.abspath(__file__))
    font_paths_bold = [
        os.path.join(_bot_dir, "font.ttf"),          # ✅ Loyiha ichidagi font (Railway)
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("admin_state") == "topup_amount":
        ...
        if card and card != "Karta raqami o'rnatilmagan":
            copy_row_btns.append(InlineKeyboardButton(text=bt("karta_nusxa"), copy_text=str(card)))
        copy_row_btns.append(InlineKeyboardButton(text=bt("miqdor_nusxa"), copy_text=str(final_amount)))
        if copy_row_btns:
            kb_rows_raw.append(copy_row_btns)
        kb_rows_raw.append([InlineKeyboardButton(text="❌ Bekor qilish", callback_data=f"cc_cancel|{pid}")])
        topup_auto_kb = InlineKeyboardMarkup(kb_rows_raw)
