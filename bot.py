...
FIREBASE_DB_URL      = os.environ.get("FIREBASE_DB_URL")      or f"https://{FIREBASE_PROJECT_ID}-default-rtdb.firebaseio.com"
...
def _firebase_url(path: str = "") -> str:
    """Firebase REST API URL hosil qiladi."""
...
def _save_firebase(data: dict, retries: int = 3) -> bool:
    """Firebase Realtime Database ga barcha ma'lumotlarni saqlaydi (PUT)."""
    try:
        token = _get_firebase_token()
        if not token:
            logger.warning("⚠️ Firebase token yo'q — saqlash o'tkazib yuborildi")
            return False
        ...
        if isinstance(movies, dict) and len(movies) == 0 and len(RAM.movies) > 0:
            logger.warning("🛑 Bo'sh movies bilan Firebase ga yozish RAD ETILDI — himoya (RAMda kinolar bor).")
            return False
        ...
        for attempt in range(retries):
            try:
                ...
                if r.status_code in (200, 201):
                    ...
                    _firebase_token_cache["token"] = None
                    token = _get_firebase_token()
                    if not token:
                        break
                    continue
                logger.error(f"Firebase save #{attempt+1} status {r.status_code}: {r.text[:100]}")
            except Exception as e:
...
def _load_firebase() -> dict | None:
    """Firebase Realtime Database dan yuklaydi (GET)."""
    try:
        token = _get_firebase_token()
        if not token:
            logger.warning("⚠️ Firebase token yo'q — yuklab bo'lmadi")
            return None
        ...
        for attempt in range(5):
            try:
                ...
                if r.status_code == 200:
                    ...
                    logger.error(f"Firebase load #{attempt+1} status {r.status_code}")
            except Exception as e:
