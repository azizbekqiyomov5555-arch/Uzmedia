...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("admin_state") == "topup_amount":
        ...
        result = await asyncio.to_thread(checkcard_create_payment, amount, order_id)

        # Eski pending to'lovni bekor qilamiz (xuddi shu miqdorda)
        if result.get("status") == "error" and "pending" in (result.get("message", "")).lower():
            ...
            result = await asyncio.to_thread(checkcard_create_payment, amount, order_id2)
        ...
        if card and card != "Karta raqami o'rnatilmagan":
            copy_row.append({"text": bt("karta_nusxa"), "copy_text": {"text": str(card)}})
        copy_row.append({"text": bt("miqdor_nusxa"), "copy_text": {"text": str(amount)}})
        kb_rows = []
        if copy_row:
            kb_rows.append(copy_row)
        kb_rows.append([ibtn("🔄 Tekshirish", data=f"cc_check|{pid}", style="primary")])
        ...
        await sm(context.bot, uid,
            ...
            f"<blockquote>⚠️ DIQQAT: {amount:,} so'mdan kam yoki ko'p tashlasangiz to'lov qabul qilinmaydi!</blockquote>\n"
        ...
        if context.application.job_queue:
            context.application.job_queue.run_repeating(
                ...
                interval=15,
                first=15,
                data={"pid": pid, "uid": uid, "order": cc_order, "amount": amount, "tries": 0},
                name=f"cc_poll_{pid}",
            )
        return
