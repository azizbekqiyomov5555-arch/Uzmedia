# -*- coding: utf-8 -*-
"""
Kino Bot - v20 (FIREBASE REALTIME DATABASE + PREMIUM + CUSTOM START)
Asosiy tuzatishlar:
1. ✅ RAM_CACHE — barcha kinolar RAMda saqlanadi, Firebase'ga fon rejimida yoziladi
2. ✅ Tezkor qidirish — RAMdan millisaniyada topadi
3. ✅ Ishga tushganda Firebase'dan RAM ga yuklab oladi (restart'da ham yo'qolmaydi!)
4. ✅ Qo'shilgan kino darhol RAMga + Firebase'ga yoziladi
5. ✅ JSONBlob ham qo'shimcha backup sifatida saqlanadi
6. ✅ Lokal /data fayl + Firebase + JSONBlob — uchlik himoya
"""
...
# Firebase Realtime Database URL (project_id dan avtomatik hosil qilinadi)
FIREBASE_PROJECT_ID  = os.environ.get("FIREBASE_PROJECT_ID")  or "uzmediyabit"
FIREBASE_DB_URL      = os.environ.get("FIREBASE_DB_URL")      or "https://uzmediyabit-default-rtdb.firebaseio.com"
FIREBASE_DB_PATH     = os.environ.get("FIREBASE_DB_PATH")     or "bot_data"

# Firebase Service Account JSON — fayl yo'li yoki environment variable
FIREBASE_CREDS_FILE  = os.environ.get("FIREBASE_CREDS_FILE")  or "uzmediyabit-firebase-adminsdk-fbsvc-6f2ff3f7fb.json"
FIREBASE_CREDS_JSON  = os.environ.get("FIREBASE_CREDS_JSON")  or ""  # JSON string sifatida ham berish mumkin

# Firebase access token (JWT) — keshlanadi, muddati tugaganda yangilanadi
_firebase_token_cache = {"token": None, "expires_at": 0}
_firebase_token_lock  = threading.Lock()

# ─── KARTA RAQAMI (MAJBURIY) ────────────────────────────────
CARD_NUMBER = "5614684700269911"
...
RAILWAY_URL = os.environ.get("RAILWAY_URL") or "https://uzmedia-production.up.railway.app"
...
# ── /data papkasini yaratish — xavfsiz fallback ───────────
_DATA_DIR = os.environ.get("DATA_DIR", "/data")
try:
    os.makedirs(_DATA_DIR, exist_ok=True)
    _tf = os.path.join(_DATA_DIR, ".write_test")
    open(_tf, "w").close()
    os.remove(_tf)
except Exception:
    _DATA_DIR = os.path.dirname(os.path.abspath(__file__)) or "."
    os.makedirs(_DATA_DIR, exist_ok=True)

LOCAL_BACKUP_FILE = os.path.join(_DATA_DIR, "db_backup.json")
LOCAL_MOVIES_FILE = os.path.join(_DATA_DIR, "movies_backup.json")
logger.info(f"📁 Data dir: {_DATA_DIR}")
...
class RamCache:
    ...
    def from_dict(self, data: dict):
        if not isinstance(data, dict):
            return
        with self._lock:
            raw_movies = data.get("movies", {}) or {}
            # movies dict bo'lishi shart — list bo'lsa o'giramiz
            if isinstance(raw_movies, list):
                raw_movies = {str(i): m for i, m in enumerate(raw_movies) if m is not None}
            self.movies = raw_movies if isinstance(raw_movies, dict) else {}
            # Har bir kino ichidagi prices ni ham tekshiramiz
            ...
            self.card_number      = CARD_NUMBER  # Har doim hardcode karta
            self.pending_payments = data.get("pending_payments", {}) or {}
            if not isinstance(self.pending_payments, dict):
...
DEFAULT_BTN = {
    ...
    "bugun_tolov":        _B('💳 Bugun to\'lov qilganlar'),
}
...
BTN_LABELS["bugun_tolov"]        = "Bugun to'lov qilganlar"
...
# ══════════════════════════════════════════════════════════
# FIREBASE REALTIME DATABASE — ASOSIY BAZA
# ══════════════════════════════════════════════════════════
#
# Firebase REST API ishlatiladi — hech qanday kutubxona shart emas.
# Service Account JSON orqali OAuth2 token olinadi.
# Token 1 soatga keshlanadi.
# ──────────────────────────────────────────────────────────

def _load_firebase_creds() -> dict | None:
    """Firebase service account credentiallarini yuklaydi."""
    # 1. Environment variable dan (JSON string)
    if FIREBASE_CREDS_JSON:
        try:
            return json.loads(FIREBASE_CREDS_JSON)
        except Exception as e:
            logger.error(f"FIREBASE_CREDS_JSON parse xato: {e}")

    # 2. Fayl yo'li bo'yicha
    for path in [
        FIREBASE_CREDS_FILE,
        os.path.join(os.path.dirname(__file__) if '__file__' in dir() else '.', FIREBASE_CREDS_FILE),
        "/app/" + FIREBASE_CREDS_FILE,
        "/data/" + FIREBASE_CREDS_FILE,
        os.path.join(_DATA_DIR, FIREBASE_CREDS_FILE),
    ]:
        if path and os.path.exists(path):
            try:
                with open(path, "r", encoding="utf-8") as f:
                    creds = json.load(f)
                logger.info(f"✅ Firebase creds yuklandi: {path}")
                return creds
            except Exception as e:
                logger.error(f"Firebase creds fayl xato ({path}): {e}")
    logger.warning("⚠️ Firebase credentials topilmadi — Firebase o'chirilgan.")
    return None


def _get_firebase_token() -> str | None:
    """Firebase OAuth2 access token oladi (keshlanadi, muddati tugaganda yangilanadi)."""
    with _firebase_token_lock:
        now = time.time()
        if _firebase_token_cache["token"] and _firebase_token_cache["expires_at"] > now + 60:
            return _firebase_token_cache["token"]

        creds = _load_firebase_creds()
        if not creds:
            return None
        ...
        try:
            ...
            # RSA imzolash — cryptography kutubxonasi bilan
            try:
                ...
                # cryptography yo'q — PyJWT orqali urinamiz
                try:
                    import jwt as _jwt
                    token_str = _jwt.encode(claims, creds["private_key"], algorithm="RS256",
                                            headers={"typ": "JWT"})
                    # PyJWT versiyasiga qarab string yoki bytes
                    if isinstance(token_str, bytes):
                        token_str = token_str.decode()
                    jwt_token = token_str
                    # Token olinganidan keyingi qism
                    r = requests.post(
                    ...
                    if r.status_code == 200:
                        ...
                        if token:
                            ...
                            _firebase_token_cache["expires_at"] = now + resp.get("expires_in", 3600)
                            return token
                    logger.error(f"Firebase token (jwt) xato: {r.status_code} {r.text[:200]}")
                    return None
                except ImportError:
                    logger.error("Firebase: 'cryptography' yoki 'PyJWT' kutubxonasi yo'q. pip install cryptography")
                    return None
            ...
            if r.status_code == 200:
                ...
                if token:
                    _firebase_token_cache["token"] = token
                    _firebase_token_cache["expires_at"] = now + resp.get("expires_in", 3600)
                    logger.info("✅ Firebase OAuth2 token olindi")
                    return token
            logger.error(f"Firebase token xato: {r.status_code} {r.text[:200]}")
            return None

        except Exception as e:
            logger.error(f"Firebase token olishda xato: {e}")
            return None


def _firebase_url(path: str = "") -> str:
    """Firebase REST API URL hosil qiladi — token siz (ochiq rules)."""
    base = FIREBASE_DB_URL.rstrip("/")
    db_path = FIREBASE_DB_PATH.strip("/")
    if path:
        return f"{base}/{db_path}/{path.strip('/')}.json"
    return f"{base}/{db_path}.json"


def _save_firebase(data: dict, retries: int = 3) -> bool:
    """Firebase Realtime Database ga saqlaydi — ochiq rules (token siz)."""
    try:
        if not FIREBASE_DB_URL:
            logger.warning("⚠️ FIREBASE_DB_URL yo'q — saqlash o'tkazib yuborildi")
            return False
        ...
        if isinstance(movies, dict) and len(movies) == 0 and len(RAM.movies) > 0:
            logger.warning("🛑 Bo'sh movies bilan Firebase ga yozish RAD ETILDI — himoya.")
            return False
        ...
        for attempt in range(retries):
            try:
                ...
                if r.status_code in (200, 201):
                    logger.info(f"✅ Firebase saqlandi ({size_kb:.1f} KB, {len(data.get('movies',{}))} kino)")
                    return True
                logger.error(f"Firebase save #{attempt+1} status {r.status_code}: {r.text[:200]}")
            except Exception as e:
                logger.error(f"Firebase save #{attempt+1} xato: {e}")
            if attempt < retries - 1:
                time.sleep(3 * (attempt + 1))
        return False
    except Exception as e:
        logger.error(f"_save_firebase umumiy xato: {e}")
        return False


def _firebase_fix_lists(data) -> dict:
    """
    Firebase numeric key larni list ga aylantiradi (masalan movies: [...]).
    Bu funksiya ularni qayta dict ga o'giradi.
    Shuningdek ichki nested list larni ham tuzatadi.
    """
    if not isinstance(data, dict):
    ...
    for key, value in data.items():
        if key in ("movies", "users", "pending_payments", "btn_texts",
                   "emoji_ids", "sub_admins", "blocked_users", "settings"):
            if isinstance(value, list):
                # Firebase list → dict: indeks raqam kalit bo'ladi
                fixed = {}
                for i, item in enumerate(value):
                    if item is not None:
                        fixed[str(i)] = item
                logger.warning(f"⚠️ Firebase '{key}' list sifatida keldi ({len(value)} ta) → dict ga o'girildi")
                result[key] = fixed
    ...
    # movies ichidagi har bir kino uchun episodes list bo'lishi kerak (dict emas)
        ...
        for code, movie in movies.items():
            if isinstance(movie, dict):
                ...
                if isinstance(eps, dict):
                    movie["episodes"] = [eps[k] for k in sorted(eps.keys(), key=lambda x: int(x) if x.isdigit() else 0)]
                    logger.warning(f"⚠️ '{code}' episodes dict edi → list ga o'girildi")
                # prices: list yoki 0 → dict
                raw_prices = movie.get("prices")
                if isinstance(raw_prices, list):
                    fixed_prices = {}
                    for i, val in enumerate(raw_prices):
                        if val is not None and val != 0 and val != "0":
                            fixed_prices[str(i + 1)] = str(val)
                    movie["prices"] = fixed_prices
                    logger.warning(f"⚠️ '{code}' prices list edi → dict ga o'girildi")
                elif not isinstance(raw_prices, dict):
                    movie["prices"] = {}
    return result


def _load_firebase() -> dict | None:
    """Firebase Realtime Database dan yuklaydi — ochiq rules (token siz)."""
    try:
        if not FIREBASE_DB_URL:
            logger.warning("⚠️ FIREBASE_DB_URL yo'q — yuklab bo'lmadi")
            return None
        ...
        for attempt in range(5):
            try:
                ...
                if r.status_code == 200:
                    data = r.json()
                    if data is None:
                        logger.info("ℹ️ Firebase bo'sh (yangi baza)")
                        return {}
                    if isinstance(data, list):
                        # Ba'zan butun data list keladi — dict ga o'giramiz
                        logger.warning(f"⚠️ Firebase root data list sifatida keldi — dict ga o'girildi")
                        fixed = {}
                    ...
                    if isinstance(data, dict):
                        # list → dict konversiyasi
                        data = _firebase_fix_lists(data)
                        movies_count = len(data.get("movies", {}) or {})
                        users_count  = len(data.get("users",  {}) or {})
                        logger.info(f"✅ Firebase dan yuklandi: {movies_count} kino, {users_count} user")
                        return data
                    logger.error(f"Firebase noto'g'ri format: {type(data)}")
                else:
                    logger.error(f"Firebase load #{attempt+1} status {r.status_code}: {r.text[:100]}")
            except Exception as e:
                logger.error(f"Firebase load #{attempt+1} xato: {e}")
            if attempt < 4:
                time.sleep(2 * (attempt + 1))
        return None
    except Exception as e:
        logger.error(f"_load_firebase umumiy xato: {e}")
        return None


# ══════════════════════════════════════════════════════════
# JSONBLOB OPERATSIYALARI (Qo'shimcha backup)
# ══════════════════════════════════════════════════════════
...
def _load_jsonblob() -> dict | None:
    """JSONBlob dan yuklash — 4 marta urinadi."""
    if not JSONBLOB_URL:
        return None
    last_err = None
    for attempt in range(4):
        ...
        if attempt < 3:
            time.sleep(2 * (attempt + 1))
    logger.error(f"❌ JSONBlob dan yuklab bo'lmadi. Oxirgi xato: {last_err}")
    return None
...
def db_initial_load():
    """
    Ishga tushganda (FIREBASE + JSONBlob + Lokal uchlik himoya):
    1. Firebase Realtime Database dan yuklaymiz (ASOSIY)
    2. JSONBlob dan yuklaymiz (backup)
    3. Lokal fayldan yuklaymiz (uchinchi backup)
    4. Uchala manbani BIRLASHTIRAMIZ — hech qanday kino yo'qolmaydi
    5. RAMga yozamiz
    6. Birlashtirilgan natijani Firebase + lokal ga qayta yozamiz

    ❗ Restart bo'lganda Firebase dan yuklash kafolati beradi!
    """
    logger.info("🔄 Ma'lumotlar yuklanmoqda (Firebase + JSONBlob + lokal)...")

    # Firebase — asosiy manba
    firebase_data = _load_firebase()
    blob          = _load_jsonblob()
    local         = _load_local()

    has_firebase = bool(firebase_data and (firebase_data.get("movies") or firebase_data.get("users")))
    has_blob     = bool(blob  and (blob.get("movies")  or blob.get("users")))
    has_local    = bool(local and (local.get("movies") or local.get("users")))

    logger.info(f"📊 Manbalar: Firebase={'✅' if has_firebase else '❌'}, "
                f"JSONBlob={'✅' if has_blob else '❌'}, "
                f"Lokal={'✅' if has_local else '❌'}")

    if not has_firebase and not has_blob and not has_local:
        logger.warning("⚠️ Hech qanday ma'lumot topilmadi — bo'sh RAM boshlanadi")
        RAM.loaded = True
        return

    # Uchala manbani birlashtirish — ko'proq qismli versiya ustunlik qiladi
    # Firebase asosiy, qolganlari qo'shimcha
    merged = _merge_db_triple(firebase_data, blob, local)
    RAM.from_dict(merged)
    EMOJI_IDS.clear()
    EMOJI_IDS.update(RAM.emoji_ids)

    fb_movies  = firebase_data.get("movies") if has_firebase else None
    blob_movies = blob.get("movies") if has_blob else None
    local_movies = local.get("movies") if has_local else None

    def _safe_eps(movies_data):
        ...
        for m in movies_data.values():
            if isinstance(m, dict):
                eps = m.get("episodes", [])
                total += len(eps) if isinstance(eps, list) else 0
        return total

    fb_eps    = _safe_eps(fb_movies)   if has_firebase else 0
    blob_eps  = _safe_eps(blob_movies) if has_blob     else 0
    local_eps = _safe_eps(local_movies) if has_local   else 0
    merged_eps = _safe_eps(RAM.movies)

    def _safe_count(src, key):
        val = (src or {}).get(key) if isinstance(src, dict) else None
        return len(val) if isinstance(val, dict) else 0

    logger.info(
        f"✅ Birlashtirildi → RAM: {len(RAM.movies)} kino, {merged_eps} qism, {len(RAM.users)} user\n"
        f"   Firebase: {_safe_count(firebase_data, 'movies')}/{fb_eps} qism\n"
        f"   JSONBlob: {_safe_count(blob, 'movies')}/{blob_eps} qism\n"
        f"   Lokal:    {_safe_count(local, 'movies')}/{local_eps} qism"
    )

    # Birlashtirilgan natijani barcha manbalarga yozib qo'yamiz
    _save_local(merged)
    # Firebase ga fon rejimida yozamiz
    threading.Thread(target=_save_firebase, args=(copy.deepcopy(merged),), daemon=True).start()
    logger.info("⏳ Birlashtirilgan ma'lumot Firebase ga sync qilinmoqda (fon)...")
    # JSONBlob ga ham backup
    if JSONBLOB_URL:
        threading.Thread(target=_save_jsonblob, args=(copy.deepcopy(merged),), daemon=True).start()


def _merge_db_triple(firebase: dict | None, blob: dict | None, local: dict | None) -> dict:
    """
    Firebase, JSONBlob va Lokal fayldagi ma'lumotlarni BIRLASHTIRADI.
    Hech qanday kino yo'qolmasligi uchun maksimal qism soni olinadi.
    Ustunlik tartibi: Firebase > JSONBlob > Lokal (sozlamalar uchun)
    """
    firebase = _firebase_fix_lists(firebase) if isinstance(firebase, dict) else {}
    blob     = blob     if isinstance(blob,     dict) else {}
    local    = local    if isinstance(local,    dict) else {}

    def _safe_movies(src):
        """movies ni xavfsiz dict sifatida olish — list bo'lsa o'girish."""
        val = src.get("movies") if isinstance(src, dict) else None
        if isinstance(val, dict):
            return val
        if isinstance(val, list):
            return {str(i): item for i, item in enumerate(val) if item is not None}
        return {}

    def _safe_users(src):
        val = src.get("users") if isinstance(src, dict) else None
        if isinstance(val, dict):
            return val
        return {}

    # Barcha manbalardagi kinolarni birlashtirish
    fb_movies    = _safe_movies(firebase)
    blob_movies  = _safe_movies(blob)
    local_movies = _safe_movies(local)

    merged_movies: dict = {}
    all_codes = set(fb_movies.keys()) | set(blob_movies.keys()) | set(local_movies.keys())

    for code in all_codes:
        candidates = [m for m in [fb_movies.get(code), blob_movies.get(code), local_movies.get(code)]
                      if isinstance(m, dict)]
        if not candidates:
            continue
        if len(candidates) == 1:
            merged_movies[code] = candidates[0]
            continue
        # Ko'proq qism bor versiyani tanlaymiz
        def _ep_count(m):
            eps = (m or {}).get("episodes", [])
            return len(eps) if isinstance(eps, list) else 0
        best = max(candidates, key=_ep_count)
        # Narxlarni birlashtir — barchadan (prices list/0 bo'lsa ham xavfsiz)
        prices = {}
        for c in candidates:
            raw = (c or {}).get("prices")
            if isinstance(raw, dict):
                ...
                # Firebase list → dict (index+1 kalit)
                for i, val in enumerate(raw):
                    if val is not None and val != 0 and val != "0":
                        prices[str(i + 1)] = str(val)
            # int/None/0 bo'lsa — o'tkazib yuboramiz
        result = dict(best)
        result["prices"] = prices
        merged_movies[code] = result

    # Foydalanuvchilar — barcha manbalardan birlashtirish, balans yo'qolmasin
    merged_users = {}
    fb_users    = _safe_users(firebase)
    blob_users  = _safe_users(blob)
    local_users = (local.get("users")     or {}) if isinstance(local.get("users"),     dict) else {}
    all_uids = set(fb_users.keys()) | set(blob_users.keys()) | set(local_users.keys())

    for uid_key in all_uids:
        fu = fb_users.get(uid_key)   or {}
        bu = blob_users.get(uid_key) or {}
        lu = local_users.get(uid_key) or {}
        # Firebase asosiy, qolganlari qo'shimcha
        base = dict(fu) if fu else (dict(bu) if bu else dict(lu))
        # Balans: barchasidan maksimalini ol
        base["balance"]     = max(int((fu or bu or lu).get("balance") or 0),
                                  int(bu.get("balance") or 0),
                                  int(lu.get("balance") or 0))
        base["topup_total"] = max(int((fu or bu or lu).get("topup_total") or 0),
                                  int(bu.get("topup_total") or 0),
                                  int(lu.get("topup_total") or 0))
        # premium_until — yangiroqni ol
        base["premium_until"] = max(
            float(fu.get("premium_until") or 0),
            float(bu.get("premium_until") or 0),
            float(lu.get("premium_until") or 0),
        )
        # paid_episodes — barcha manbalardan birlashtir
        paid = {}
        for src in [bu, lu, fu]:  # Firebase oxirgi — ustun
            paid.update((src or {}).get("paid_episodes") or {})
        base["paid_episodes"] = paid
        merged_users[uid_key] = base

    def pick3(key, default):
        """Firebase > JSONBlob > Lokal ustunligi."""
        for src in [firebase, blob, local]:
    ...
    return {
        "movies":           merged_movies,
        "users":            merged_users,
        "channels":         pick3("channels", []),
        "simple_links":     pick3("simple_links", []),
        "card_number":      pick3("card_number", ""),
        "pending_payments": pick3("pending_payments", {}),
        "settings":         pick3("settings", {
            "install_file_id": None, "install_video_id": None, "kino_kanal_url": "",
        }),
        "stats":            pick3("stats", {"total_views": 0}),
        "btn_texts":        pick3("btn_texts", {}),
        "emoji_ids":        pick3("emoji_ids", {}),
        "sub_admins":       pick3("sub_admins", {}),
        "blocked_users":    pick3("blocked_users", {}),
    }
...
JSONBLOB_DEBOUNCE = 30.0   # soniya — qism yuborish tugagandan keyin
...
async def _do_jsonblob_save() -> bool:
    """Firebase (asosiy) + JSONBlob (backup) ga yozish (status ham yangilanadi)."""
    async with _ensure_lock():
        data = RAM.to_dict()
        # Firebase — parallel yozamiz
        firebase_ok = await asyncio.to_thread(_save_firebase, data)
        # JSONBlob — parallel backup
        jsonblob_ok = await asyncio.to_thread(_save_jsonblob, data) if JSONBLOB_URL else True

        # Kamida bittasi muvaffaqiyatli bo'lsa — OK
        ok = firebase_ok or jsonblob_ok
        ...
        if ok:
            ...
            if not firebase_ok:
                logger.warning(f"⚠️ Firebase xato, lekin JSONBlob OK — {now_str}")
            elif not jsonblob_ok:
                logger.info(f"ℹ️ JSONBlob xato, lekin Firebase OK — {now_str}")
        else:
...
def save_sync():
    """Sinxron (thread) — lokal + Firebase + JSONBlob fon rejimida."""
    data = RAM.to_dict()
    _save_local(data)
    threading.Thread(target=_save_firebase, args=(copy.deepcopy(data),), daemon=True).start()
    if JSONBLOB_URL:
...
def generate_movies_image(movie_slice: list, page: int = 1, total_pages: int = 1,
    ...
    # ── Ranglar palitasi (qorong'i kino uslubi) ──────────────
    BG_DARK      = (8,  11,  24)
    BG_CARD      = (16, 21,  44)
    BG_CARD2     = (13, 18,  38)
    GOLD         = (255, 200, 50)
    ...
    WHITE        = (255, 255, 255)
    GREY_LIGHT   = (180, 185, 210)
    GREEN_SOFT   = (50,  210, 130)
    ACCENT_COLORS = [
        (80, 140, 255),
        (50, 210, 130),
        (255, 100, 140),
    ...
    n     = len(movie_slice)
    img_h = HEADER_H + TOP_PAD + n * (CARD_H + GAP) + GAP + FOOTER_H

    img  = Image.new("RGB", (IMG_W, img_h), BG_DARK)
    draw = ImageDraw.Draw(img)

    # Fon nuqtali tekstura
    for yy in range(0, img_h, 18 * S):
        for xx in range(0, IMG_W, 18 * S):
            draw.ellipse([xx, yy, xx + S, yy + S], fill=(20, 26, 54))
    ...
    fnt_h1     = _fnt(58 * S)
    fnt_h2     = _fnt(30 * S, bold=False)
    fnt_num    = _fnt(44 * S)
    fnt_title  = _fnt(38 * S)
    fnt_code   = _fnt(28 * S)
    fnt_sub    = _fnt(24 * S, bold=False)
    fnt_footer = _fnt(26 * S, bold=False)
    ...
    for i in range(HEADER_H):
        ...
        g = int(28 + (15 - 28) * t)
        b = int(70 + (35 - 70) * t)
        draw.line([(0, i), (IMG_W, i)], fill=(r, g, b))

    # Oltin ajratuvchi chiziq
    draw.rectangle([(0, HEADER_H - 6 * S), (IMG_W, HEADER_H)], fill=GOLD)

    # Kino lenta nuqtalari — yuqori
    dot_r = 10 * S
    for xi in range(dot_r * 2, IMG_W - dot_r * 2, dot_r * 4):
        draw.ellipse([xi - dot_r // 2, 6 * S, xi + dot_r // 2, 6 * S + dot_r], fill=GOLD_DARK)

    # Sarlavha
    title_txt = "BARCHA KINOLAR"
    tw, th = _tw(draw, title_txt, fnt_h1)
    tx0 = (IMG_W - tw) // 2
    ty0 = (HEADER_H - th) // 2 - 18 * S
    draw.text((tx0 + 4 * S, ty0 + 4 * S), title_txt, fill=(0, 0, 0), font=fnt_h1)
    draw.text((tx0, ty0), title_txt, fill=GOLD, font=fnt_h1)

    # Subtekst
    if total_pages > 1:
        sub_txt = f"Sahifa {page}/{total_pages}   |   Jami {total_count} ta kino"
    else:
        sub_txt = f"Jami {total_count} ta kino mavjud"
    sw, _ = _tw(draw, sub_txt, fnt_h2)
    draw.text(((IMG_W - sw) // 2, ty0 + th + 10 * S), sub_txt, fill=GREY_LIGHT, font=fnt_h2)

    # Burchak yulduzlari
    for sx, sy in [(80 * S, HEADER_H // 2), (IMG_W - 80 * S, HEADER_H // 2),
                   (160 * S, HEADER_H // 3), (IMG_W - 160 * S, HEADER_H // 3)]:
        sr = 6 * S
    ...
    for idx, (code, movie) in enumerate(movie_slice):
        ...
        col_dim = tuple(max(0, c - 60) for c in col)

        # Soya
        draw.rounded_rectangle(
            [x0 + 6 * S, y0 + 6 * S, x1 + 6 * S, y1 + 6 * S],
            radius=RADIUS, fill=(4, 6, 14))

        # Karta foni
        card_fill = BG_CARD if idx % 2 == 0 else BG_CARD2
        draw.rounded_rectangle([x0, y0, x1, y1], radius=RADIUS, fill=card_fill)

        # Chap rangli chiziq
        draw.rounded_rectangle([x0, y0, x0 + STRIP_W, y1], radius=RADIUS, fill=col)

        # O'ng yuqori kichik nuqta
        draw.ellipse([x1 - 20 * S, y0 + 8 * S, x1 - 8 * S, y0 + 20 * S], fill=col)

        # Raqam badji
        bx = x0 + STRIP_W + 22 * S + BADGE_R
        by = y0 + CARD_H // 2
        draw.ellipse([bx - BADGE_R, by - BADGE_R, bx + BADGE_R, by + BADGE_R], fill=col_dim)
        inner = BADGE_R - 8 * S
        draw.ellipse([bx - inner, by - inner, bx + inner, by + inner], fill=col)
        num_txt = str(start_offset + idx + 1)
        nw, nh  = _tw(draw, num_txt, fnt_num)
        draw.text((bx - nw // 2, by - nh // 2 - 2 * S), num_txt, fill=WHITE, font=fnt_num)

        # Kino nomi
        tx = bx + BADGE_R + 24 * S
        raw_title = _strip_html(movie.get("title", code))
        if len(raw_title) > 36: raw_title = raw_title[:34] + "..."
        title_y = y0 + 26 * S
        draw.text((tx + 2 * S, title_y + 2 * S), raw_title, fill=(0, 0, 0), font=fnt_title)
        draw.text((tx, title_y), raw_title, fill=WHITE, font=fnt_title)

        # Kod / qism / ko'rishlar
        ep_count    = len(movie.get("episodes", []) or [])
        views_total = sum((movie.get("views") or {}).values())
        info_y = y0 + 90 * S

        code_txt = f"  {code}  "
        cw, ch_  = _tw(draw, code_txt, fnt_code)
        cp       = 6 * S
        draw.rounded_rectangle(
            [tx - cp, info_y - cp // 2, tx + cw + cp, info_y + ch_ + cp // 2],
            radius=8 * S, fill=col_dim)
        draw.text((tx, info_y), code_txt, fill=WHITE, font=fnt_code)
        after_code = tx + cw + cp + 18 * S

        ep_txt = f"  {ep_count} qism  "
        ew, eh  = _tw(draw, ep_txt, fnt_sub)
        draw.rounded_rectangle(
            [after_code - 6 * S, info_y, after_code + ew + 6 * S, info_y + eh + 4 * S],
            radius=6 * S, fill=(30, 45, 80))
        draw.text((after_code, info_y + 2 * S), ep_txt, fill=GREEN_SOFT, font=fnt_sub)
        after_ep = after_code + ew + 18 * S

        draw.text((after_ep, info_y + 2 * S), f"  {views_total} korilgan",
                  fill=GREY_LIGHT, font=fnt_sub)

        # O'ng tomon uch nuqta
        for di in range(3):
            dcy = y0 + CARD_H // 2 + (di - 1) * 22 * S
            draw.ellipse([x1 - 28 * S, dcy - 5 * S, x1 - 18 * S, dcy + 5 * S], fill=col_dim)
    ...
    for i in range(FOOTER_H):
        ...
        g = int(14 + (28 - 14) * t)
        b = int(35 + (70 - 35) * t)
        draw.line([(0, fy0 + i), (IMG_W, fy0 + i)], fill=(r, g, b))

    draw.rectangle([(0, fy0), (IMG_W, fy0 + 5 * S)], fill=GOLD)

    if total_pages > 1:
        start_n = (page - 1) * KINO_LIST_PAGE_SIZE + 1
        end_n   = min(start_n + len(movie_slice) - 1, total_count)
    ...
    fw, fh_ = _tw(draw, f_text, fnt_footer)
    draw.text(((IMG_W - fw) // 2, fy0 + (FOOTER_H - fh_) // 2),
              f_text, fill=GREY_LIGHT, font=fnt_footer)

    # Kino lenta nuqtalari — pastki
    for xi in range(dot_r * 2, IMG_W - dot_r * 2, dot_r * 4):
        draw.ellipse([xi - dot_r // 2, img_h - 6 * S - dot_r,
                      xi + dot_r // 2, img_h - 6 * S], fill=GOLD_DARK)
...
async def _send_kino_list_page(bot, chat_id: int, page: int = 0, query=None):
    ...
    # ── PIL bilan chiroyli rasm yasash ──────────────────────
    img_buf = None
    if PIL_AVAILABLE:
        try:
    ...
    # ── PIL rasm bor bo'lsa — edit yoki yangi yuborish ─────
    ...
    # ── PIL ishlamasa — posterlari bor kinolarni media group ─
    from telegram import InputMediaPhoto as _IMP
    poster_items = [(c, m) for c, m in slice_items if m.get("poster_file_id")]

    if poster_items:
        # Media group: har bir kino posterini caption bilan yuboramiz
        media_group = []
        for i, (c, m) in enumerate(poster_items):
            ep_c   = len(m.get("episodes", []) or [])
            title  = _strip_html(m.get("title", c))
            cap    = (f"🎬 <b>{title}</b>\n"
                      f"📺 {ep_c} ta qism\n"
                      f"📋 Kod: <code>{c}</code>")
            media_group.append(_IMP(
                media=m["poster_file_id"],
        ...
        try:
            ...
            logger.warning(f"media_group xato: {e}")

        # Postersiz kinolar + navigatsiya tugmalari
        no_poster = [(c, m) for c, m in slice_items if not m.get("poster_file_id")]
        extra = ""
        if no_poster:
            lines = []
            for c, m in no_poster:
                ep_c = len(m.get("episodes", []) or [])
                lines.append(f"🎬 <b>{_strip_html(m.get('title', c))}</b> — <code>{c}</code> ({ep_c} qism)")
            extra = "\n\n" + "\n".join(lines)

        page_info = f"📄 {page+1}/{total_pages} sahifa  |  Jami: {total_count} ta kino"
        await bot.send_message(
            chat_id=chat_id,
            text=f"{page_info}{extra}\n\n{caption}",
            parse_mode="HTML",
            reply_markup=kb)
        return

    # ── Poster ham yo'q — matn ro'yxat ─────────────────────
    lines = []
    for i, (c, m) in enumerate(slice_items, start=start+1):
        ep_c = len(m.get("episodes", []) or [])
        lines.append(f"{i}. 🎬 <b>{_strip_html(m.get('title', c))}</b>\n"
                     f"    📋 Kod: <code>{c}</code>  |  {ep_c} ta qism")
    page_info = f"📄 {page+1}/{total_pages} sahifa  |  Jami: {total_count} ta kino\n\n"
    text_list = page_info + "\n\n".join(lines) + f"\n\n{caption}"
    await bot.send_message(chat_id=chat_id,
        text=text_list, parse_mode="HTML", reply_markup=kb)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("cc_check|"):
        ...
        cc_status = (cc_data.get("status")
                     ...
                     or cc_data.get("payment_status")
                     or result.get("status")
                     or result.get("state")
                     or result.get("payment_status")
                     or "")
        cc_status = str(cc_status).lower().strip()
        is_paid   = cc_status in ("paid", "success", "completed", "1", "true")
        is_cancel = cc_status in ("cancel", "cancelled", "failed", "rejected", "0")
        if is_paid:
            pay["cc_status"] = "paid"
    ...
    if data.startswith("finish_movie|"):
        ...
        await sm(context.bot, uid, "💾 Firebase + JSONBlob bazaga saqlanmoqda, kuting...")
        ok = await save_now()
        if not ok:
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("admin_state") == "topup_amount":
        ...
        # ── Unique pay_amount: bir xil miqdor bir vaqtda 2 foydalanuvchi yuborsa
        # CheckCard chalkashib qolmasin uchun 1–99 so'm random qo'shiladi.
        # Foydalanuvchi balance_amount (kiritgan raqami) oladi,
        # lekin kartaga pay_amount o'tkazadi.
        import random as _rnd
        suffix = _rnd.randint(1, 99)
        pay_amount = amount + suffix   # masalan 5000 → 5037

        # ── CheckCard API orqali to'lov yaratish ──
        wait_msg = await sm(context.bot, uid, "⏳ To'lov yaratilmoqda...")
        order_id = f"t{uid}{int(time.time())}"
        result = await asyncio.to_thread(checkcard_create_payment, pay_amount, order_id)

        # Eski pending to'lovni bekor qilamiz (xuddi shu foydalanuvchi uchun)
        if result.get("status") == "error" and "pending" in (result.get("message", "")).lower():
            ...
            suffix2 = _rnd.randint(1, 99)
            pay_amount = amount + suffix2
            order_id2 = f"t{uid}{int(time.time())}r"
            result = await asyncio.to_thread(checkcard_create_payment, pay_amount, order_id2)
        ...
        if result.get("status") != "success":
            card = RAM.card_number or CARD_NUMBER
        ...
        RAM.pending_payments[pid] = {
            ...
            "user_id":        uid,
            "amount":         amount,      # balansgа qo'shiladigan miqdor
            "pay_amount":     pay_amount,  # kartaga o'tkazish kerak bo'lgan miqdor
            "cc_order":       cc_order,
            "cc_status":      "pending",
        ...
        if card and card != "Karta raqami o'rnatilmagan":
            copy_row.append({"text": bt("karta_nusxa"), "copy_text": {"text": str(card)}})
        # Nusxalash tugmasida pay_amount (to'lanadigan miqdor) ko'rsatiladi
        copy_row.append({"text": bt("miqdor_nusxa"), "copy_text": {"text": str(pay_amount)}})
        kb_rows = []
        if copy_row:
            kb_rows.append(copy_row)
        # Tekshirish tugmasi YO'Q — polling avtomatik ishlaydi
        ...
        await sm(context.bot, uid,
            ...
            f"💰 Balansingizga qo'shiladi: <b>{amount:,} so'm</b>\n"
            f"💳 Kartaga o'tkazish: <b>{pay_amount:,} so'm</b>\n"
            f"📋 Order ID: <code>{cc_order}</code>\n\n"
            f"{card_line}"
            f"⬆️ <b>Yuqoridagi karta raqamiga aynan <u>{pay_amount:,} so'm</u> o'tkazing!</b>\n"
            f"<i>To'lovni amalga oshirgach bot avtomatik balansni to'ldiradi.</i>\n\n"
            f"<blockquote>⚠️ DIQQAT: {pay_amount:,} so'mdan kam yoki ko'p tashlasangiz to'lov qabul qilinmaydi!</blockquote>\n"
        ...
        if context.application.job_queue:
            context.application.job_queue.run_repeating(
                ...
                interval=20,
                first=20,
                data={"pid": pid, "uid": uid, "order": cc_order,
                      "amount": amount, "pay_amount": pay_amount, "tries": 0},
                name=f"cc_poll_{pid}",
            )
        return
...
async def admin_buttons(update, context, text: str):
    ...
    if _btn_match("bugun_tolov"):
        if not is_any_admin(uid):
            return
        today_str = datetime.now().strftime("%Y-%m-%d")
        payments  = RAM.pending_payments or {}
        users_db  = RAM.users or {}

        # Bugun tasdiqlangan to'lovlarni yig'amiz
        today_pays = []
        for pid, pay in payments.items():
            approved_at = pay.get("approved_at") or ""
            cc_approved = (pay.get("cc_status") == "paid")
            is_today    = approved_at.startswith(today_str)
            if not is_today and cc_approved:
                cc_paid_at = pay.get("cc_paid_at") or approved_at
                is_today   = str(cc_paid_at).startswith(today_str)
            if not is_today:
                continue
            pay_status = pay.get("status", "")
            if pay_status not in ("approved",) and not cc_approved:
                continue
            today_pays.append((pid, pay))

        if not today_pays:
            await sm(context.bot, uid,
                f"📊 <b>Bugun to'lov qilganlar</b>\n\n"
                f"📅 Sana: <b>{today_str}</b>\n\n"
                "❌ <i>Bugun hali tasdiqlangan to'lov yo'q.</i>")
            return

        # Foydalanuvchi bo'yicha guruhlash
        user_pay_map: dict = {}
        for pid, pay in today_pays:
            u_id_str = str(pay.get("user_id", ""))
            user_pay_map.setdefault(u_id_str, []).append(pay)

        lines = []
        total_sum   = 0
        total_count = 0
        num = 0
        for u_id_str, pays in user_pay_map.items():
            u_info  = users_db.get(u_id_str) or {}
            u_name  = u_info.get("name") or f"ID:{u_id_str}"
            u_uname = u_info.get("username") or ""
            uname_part = f' (@{u_uname})' if u_uname else ""

            for pay in pays:
                num        += 1
                amount      = int(pay.get("amount") or 0)
                pay_type    = pay.get("type", "")
                code        = pay.get("code") or ""
                ep          = pay.get("ep") or ""
                total_sum  += amount
                total_count += 1

                if pay_type in ("topup", "topup_checkcard"):
                    what = "💰 <i>Balans to'ldirish</i>"
                else:
                    movie   = RAM.movies.get(str(code).upper()) if code else None
                    m_title = movie.get("title", code) if movie else (code or "?")
                    what    = f'🎬 <b>{m_title}</b>' + (f' — <b>{ep}-qism</b>' if ep else '')

                # To'lov usuli
                if pay_type == "topup_checkcard":
                    method = "💳 CheckCard (avtomatik)"
                elif pay_type == "topup":
                    method = "🖼 Chek orqali"
                else:
                    method = "🖼 Chek orqali"
                ...
                lines.append(
                    f"┌ <b>{num}-to'lov</b>\n"
                    f"│ 👤 <b>{u_name}</b>{uname_part}\n"
                    f"│ 🆔 <code>{u_id_str}</code>\n"
                    f"│ 💵 <b>{amount:,} so'm</b>\n"
                    f"│ {what}\n"
                    f"└ {method}"
                )
        ...
        header = (
            f"📊 <b>Bugun to'lov qilganlar</b>\n"
            f"📅 Sana: <b>{today_str}</b>\n"
            f"👥 Jami: <b>{total_count} ta to'lov</b>\n"
            f"💰 Umumiy summa: <b>{total_sum:,} so'm</b>\n"
            f"{'━'*26}\n\n"
        )
        body      = "\n\n".join(lines)
        ...
        if len(full_text) <= CHUNK:
            await sm(context.bot, uid, full_text)
        else:
            await sm(context.bot, uid, header.rstrip())
            chunk_lines: list = []
            chunk_len         = 0
            for line in lines:
                line_len = len(line) + 2
                if chunk_len + line_len > CHUNK and chunk_lines:
                    await sm(context.bot, uid, "\n\n".join(chunk_lines))
                    chunk_lines = []
                    chunk_len   = 0
...
async def admin_state_handler(update, context, text: str) -> bool:
    ...
    if state == "set_card":
        RAM.card_number = CARD_NUMBER  # Har doim asosiy karta ishlatiladi
        await schedule_save()
        context.user_data.pop("admin_state", None)
        await sm(context.bot, uid, f"✅ Karta: <code>{CARD_NUMBER}</code>", admin_menu_kb(uid))
        return True
...
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_any_admin(uid) and state == "add_ep_video":
        ...
        # Firebase + JSONBlob ga TEGMAYMIZ — periodik sync (har 2 daqiqada) qiladi.
        # Bu 100 ta qism ketma-ket yuborishda botni sekinlashtirmaydi.
        await save_ram_only()
        ...
        await sm(context.bot, uid,
            ...
            f"Jami qismlar: <b>{ep_num} ta</b>\n\n"
            f"📹 Yana video yuboring yoki <b>Tugatish</b> tugmasini bosing.\n"
            f"<i>RAMda va lokal faylda saqlandi. Firebase+JSONBlob — Tugatish bosganda.</i>",
            movie_added_kb(code))
...
_BOT_APP = None
_BOT_LOOP = None   # ← Asosiy asyncio event loop (webhook uchun)
...
class _WebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            # ✅ TO'G'RI: asosiy bot event loop orqali ishlaymiz
            # (yangi loop ochish noto'g'ri — bot boshqa loopda ishlaydi)
            if _BOT_LOOP and not _BOT_LOOP.is_closed():
                future = asyncio.run_coroutine_threadsafe(
                    _handle_checkcard_webhook(data), _BOT_LOOP
                )
                try:
                    future.result(timeout=15)  # 15 soniya kutamiz
                except Exception as e:
                    logger.error(f"Webhook async xato: {e}")
            else:
                # Loop hali tayyor emas — sinxron saqlaymiz
                _handle_checkcard_webhook_sync(data)
        except Exception as e:
...
def _handle_checkcard_webhook_sync(data: dict):
    """
    Webhook sinxron versiyasi — bot loop tayyor bo'lmagan paytda.
    Faqat RAMga yozadi va lokal saqlanadi. Xabar yuborilmaydi.
    """
    try:
        ...
        for pid, pay in list(RAM.pending_payments.items()):
            if (pay.get("type") == "topup_checkcard"
                ...
                pay["cc_status"] = "paid"
                pay_amount = int(pay.get("amount") or amount)
                uid_str = str(pay.get("user_id"))
                u_data = RAM.ensure_user(uid_str)
                u_data["balance"]     = int(u_data.get("balance") or 0) + pay_amount
                u_data["topup_total"] = int(u_data.get("topup_total") or 0) + pay_amount
                _save_local(RAM.to_dict())
                logger.info(f"✅ Webhook sync: {uid_str} ga {pay_amount:,} so'm qo'shildi")
                break
    except Exception as e:
        logger.error(f"_handle_checkcard_webhook_sync xato: {e}")
...
async def _checkcard_poll_job(context):
    ...
    try:
        ...
        amount   = job_data["amount"]       # balansgа qo'shiladigan miqdor
        pay_amount = job_data.get("pay_amount", amount)  # kartaga o'tkazilgan miqdor
        ...
        cc_data = result.get("data", {}) or {}
        # CheckCard turli formatlarda qaytarishi mumkin — barchasini tekshiramiz
        cc_status = (cc_data.get("status")
                     or cc_data.get("state")
                     or cc_data.get("payment_status")
                     or result.get("status")
                     ...
                     or "")
        cc_status = str(cc_status).lower().strip()
        # "paid", "success", "completed" — to'langan holatlari
        is_paid = cc_status in ("paid", "success", "completed", "1", "true")
        # "cancel", "cancelled", "failed", "rejected" — bekor qilish holatlari
        is_cancel = cc_status in ("cancel", "cancelled", "failed", "rejected", "0")
        ...
        if is_paid:
            ...
            await save_now()
            try:
                paid_line = (f"💳 To'langan: <b>{pay_amount:,} so'm</b>\n" if pay_amount != amount else "")
                await context.bot.send_message(
                    uid,
                    f"✅ <b>TO'LOV TASDIQLANDI!</b>\n\n"
                    f"{paid_line}"
...
def main():
    ...
    # ── Firebase credentials faylini /data ga ko'chirish (Railway Volume uchun) ──
    _fb_src = FIREBASE_CREDS_FILE
    _fb_dst = os.path.join(_DATA_DIR, os.path.basename(FIREBASE_CREDS_FILE))
    if os.path.exists(_fb_src) and not os.path.exists(_fb_dst):
        try:
            import shutil
            shutil.copy2(_fb_src, _fb_dst)
            logger.info(f"✅ Firebase creds → {_fb_dst}")
        except Exception as _e:
            logger.warning(f"Firebase creds copy xato: {_e}")
    ...
    # ── Har 2 daqiqada Firebase + JSONBlob ga sinxron saqlash ────────
    async def _periodic_sync(context_job):
        try:
            ...
            _save_local(data)
            # Firebase — asosiy
            fb_ok   = await asyncio.to_thread(_save_firebase, data)
            # JSONBlob — backup
            jb_ok   = await asyncio.to_thread(_save_jsonblob, data) if JSONBLOB_URL else True
            ok      = fb_ok or jb_ok
            now_str = datetime.now().strftime("%H:%M:%S")
            if ok:
                DB_STATUS.update({
                ...
                if was_down:
                    try:
                        await context_job.bot.send_message(
                            ...
                            f"🔥 Firebase: {'✅' if fb_ok else '❌'}  📦 JSONBlob: {'✅' if jb_ok else '❌'}\n"
                ...
                if DB_STATUS["fail_count"] == 2:
                    try:
                        await context_job.bot.send_message(
                            ADMIN_ID,
                            f"⚠️ <b>Firebase VA JSONBlob ishlamayapti!</b>\n{now_str}\n"
                            f"Bot RAMdan ishlayapti (ma'lumotlar lokal saqlanadi).",
                            parse_mode="HTML")
                    except: pass
            status = "✅" if ok else "⚠️"
    ...
    async def _startup_notify(context_job):
        try:
            ...
            fb_status = "🔥 Firebase: ✅ ishlayapti" if fb_ok else "🔥 Firebase: ❌ xato"
            jb_status = "📦 JSONBlob: ✅ ishlayapti" if jb_ok else "📦 JSONBlob: ❌ xato"
            webhook_url = f"{RAILWAY_URL}{CHECKCARD_WEBHOOK_PATH}"
            await context_job.bot.send_message(
                ADMIN_ID,
                f"🚀 <b>Bot v20 (Firebase) Railway da ishga tushdi!</b>\n\n"
                ...
                f"✅ Kinolar restart bo'lganda ham saqlanadi (Firebase kafolati)!",
                parse_mode="HTML")
        except Exception as e:
    ...
    if app.job_queue:
        ...
        app.job_queue.run_repeating(_periodic_sync, interval=600, first=120)
        logger.info("🔄 Periodik sync yoqildi (har 10 daqiqada)")

    logger.info(f"🚀 Bot v20 (Firebase) Railway ishga tushdi! RAM: {len(RAM.movies)} kino, {len(RAM.users)} user")
    app.run_polling(
