...
def checkcard_create_payment(amount: int, order_id: str = None) -> dict:
    ...
    try:
        ...
        logger.info(f"[CC] create javob ({r.status_code}): {r.text[:500]}")
        data = r.json()

        # CheckCard webhook formatidan: {"shopkey":..,"amount":..,"order":..,"date":..}
        # Create javobi turli formatlarda kelishi mumkin — normalizatsiya
        raw_status = str(data.get("status", "")).lower().strip()
        raw_success = data.get("success")
        # order_code — SQL da shu nom bilan saqlanadi
        order_val = (data.get("order")
                     or data.get("order_code")
                     or (data.get("data") or {}).get("order")
                     or (data.get("data") or {}).get("order_code"))

        is_ok = (
            raw_status in ("success", "ok", "1", "true", "created", "pending")
            or raw_success is True
            or raw_success == 1
            or bool(order_val)
        )
        is_err = (
            raw_status in ("error", "fail", "failed", "0", "false")
            or (raw_success is False or raw_success == 0)
        ) and not bool(order_val)

        if is_ok:
            data["status"] = "success"
            data["order"]  = order_val or order_id
        elif is_err:
            data["status"] = "error"

        logger.info(f"[CC] create normalized: status={data.get('status')}, order={data.get('order')}")
        return data
    except Exception as e:
        logger.error(f"[CC] create xato: {e}")
...
def checkcard_check_payment(order: str) -> dict:
    ...
    try:
        url = (f"{CHECKCARD_BASE_URL}?method=check"
               ...
               f"&order={order}")
        r = requests.get(url, timeout=20)
        logger.info(f"[CC] check javob ({r.status_code}): {r.text[:300]}")
        data = r.json()

        # SQL: status VARCHAR(12) — qiymatlar: "pending", "paid", "cancelled"
        raw_status = str(
            data.get("status")
        ...
        # Webhook formatida kelsa — amount va order bor bo'lsa "paid" hisoblaymiz
        # {"shopkey":..,"amount":..,"order":..,"date":..}
        has_date = bool(data.get("date") or (data.get("data") or {}).get("date"))
        if has_date and not raw_status:
            raw_status = "paid"

        # Normalizatsiya
        if raw_status in ("paid", "success", "completed", "1", "true"):
            data["status"] = "paid"
        elif raw_status in ("pending", "created", "waiting"):
            data["status"] = "pending"
        elif raw_status in ("cancelled", "cancel", "failed", "rejected", "0"):
            data["status"] = "cancelled"

        logger.info(f"[CC] check normalized: order={order} status={data.get('status')}")
        return data
    except Exception as e:
        logger.error(f"[CC] check xato: {e}")
        return {"status": "error"}
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("cc_check|"):
        ...
        # checkcard_check_payment normalizatsiya qilgan
        cc_status = str(result.get("status") or "").lower().strip()
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("admin_state") == "topup_amount":
        ...
        # ── Eski pending to'lovlarni bekor qilamiz ──
        for pid_old, pay_old in list(RAM.pending_payments.items()):
            if (pay_old.get("type") == "topup_checkcard"
                ...
                logger.info(f"[TOPUP] Eski pending bekor: {pid_old}")

        # ── CheckCard API orqali to'lov yaratish ──
        import random
        order_id = f"t{uid}{int(time.time())}{random.randint(100,999)}"
        ...
        logger.info(f"[TOPUP] uid={uid} amount={amount} result={result}")
...
async def _checkcard_poll_job(context):
    ...
    try:
        ...
        # SQL status: "paid", "pending", "cancelled"
        # checkcard_check_payment allaqachon normalizatsiya qilgan
        cc_status = str(result.get("status") or "").lower().strip()
        is_paid   = cc_status in ("paid", "success", "completed", "1", "true")
        is_cancel = cc_status in ("cancel", "cancelled", "failed", "rejected", "0")
