...
# ── Spam himoya: qo'llanma video cooldown ─────────────────
_INSTALL_COOLDOWN: dict = {}  # {uid_str: last_sent_time}
_INSTALL_COOLDOWN_SEC = 30    # 30 soniya kutish
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "start_qollanma":
        ...
        if vid_id:
            ...
            dasturchi_kb = InlineKeyboardMarkup([[
                InlineKeyboardButton("👨‍💻 Dasturchi lichkasi", url="https://t.me/DramlarUz_help", style="danger")
            ]])
            try:
                await sv(context.bot, uid, vid_id, cap, markup=dasturchi_kb)
            except Exception as e:
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if _main_btn("install"):
        # Spam himoya — 30 soniya cooldown
        now = time.time()
        last = _INSTALL_COOLDOWN.get(str(uid), 0)
        if now - last < _INSTALL_COOLDOWN_SEC:
            wait = int(_INSTALL_COOLDOWN_SEC - (now - last))
            try:
                await msg.reply_text(
                    f"⏳ Iltimos, <b>{wait}</b> soniya kuting.",
                    parse_mode="HTML"
                )
            except Exception:
                pass
            return
        _INSTALL_COOLDOWN[str(uid)] = now
        ...
        await sv(context.bot, uid, v_id, cap, markup=dasturchi_kb)
        return
