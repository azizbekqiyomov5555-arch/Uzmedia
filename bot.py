...
def checkcard_create_payment(amount: int, order_id: str = None) -> dict:
    ...
    try:
        ...
        # ── Status normalizatsiya ──────────────────────────────
        # CheckCard API turli formatlarda qaytarishi mumkin:
        #   {"status": "success"}, {"status": 1}, {"success": true},
        #   {"status": "ok"}, {"data": {"order": "..."}} — barchasini tekshiramiz
        raw_status = str(data.get("status", "")).lower().strip()
        raw_success = data.get("success")
        has_order   = bool(data.get("order") or (data.get("data") or {}).get("order"))

        is_ok = (
            raw_status in ("success", "ok", "1", "true", "created")
        ...
        if is_ok:
            # order ni olish — turli joylarda bo'lishi mumkin
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("admin_state") == "topup_amount":
        ...
        cc_order = result.get("order") or (result.get("data") or {}).get("order") or order_id
