...
JSONBLOB_BACKUP_URL = "https://jsonblob.com/api/jsonBlob/019df4aa-10b1-725c-83f5-9901ab2db9b6"
...
def _fetch_one_blob(url: str) -> dict | None:
    """Bitta JSONBlob URL dan yuklash."""
    last_err = None
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if isinstance(data, dict):
                    logger.info(f"✅ Blob yuklandi: {len(data.get('movies', {}))} kino, "
                                f"{len(data.get('users', {}))} user")
                    return data
            else:
                logger.error(f"Blob status {r.status_code}")
        except Exception as e:
            last_err = e
            logger.error(f"Blob xato #{attempt+1}: {e}")
        if attempt < 2:
...
def _load_jsonblob() -> dict | None:
    ...
    # Backup blob — asosiy bo'sh bo'lsa yuklaymiz
    backup_data = None
    backup_url = globals().get("JSONBLOB_BACKUP_URL", "")
    if backup_url and backup_url != JSONBLOB_URL:
        if not (main_data or {}).get("movies"):
            logger.info("ℹ️ Asosiy blob bo'sh — backup tekshirilmoqda...")
    ...
    if backup_data and main_data:
        merged = {**backup_data, **main_data}
        merged["movies"] = {**backup_data.get("movies", {}), **main_data.get("movies", {})}
        merged["users"]  = {**backup_data.get("users",  {}), **main_data.get("users",  {})}
        logger.info(f"🔗 Birlashtirildi: {len(merged['movies'])} kino")
        threading.Thread(
            target=lambda: requests.put(
                JSONBLOB_URL,
                headers={"Content-Type": "application/json", "Accept": "application/json"},
                data=json.dumps(merged), timeout=60
            ), daemon=True
        ).start()
        return merged
    return backup_data or main_data
...
def db_initial_load():
    ...
    if not RAM.card_number and CARD_NUMBER:
        ...
        logger.info(f"💳 Karta raqami yuklandi: {CARD_NUMBER}")
...
def generate_movies_image(movie_slice: list, page: int = 1, total_pages: int = 1,
    ...
    font_paths_bold = [
        os.path.join(_bot_dir, "font.ttf"),
        ...
        try:
            ...
            if ok:
                ...
                if DB_STATUS["fail_count"] >= 2:
                    DB_STATUS.update({"storage_ok": False, "ram_only": True})
                if DB_STATUS["fail_count"] == 2:
                    logger.warning("⚠️ JSONBlob ishlamayapti — bot RAMdan ishlayapti")
            status = "✅" if ok else "⚠️"
