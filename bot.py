...
def db_initial_load():
    ...
    fb_eps    = sum(len(m.get("episodes", []) or []) for m in (firebase_data.get("movies") or {}).values()) if has_firebase else 0
    ...
    logger.info(
        ...
        f"   Firebase: {len((firebase_data or {}).get('movies',{}))}/{fb_eps} qism\n"
        f"   JSONBlob: {len((blob or {}).get('movies',{}))}/{blob_eps} qism\n"
        f"   Lokal:    {len((local or {}).get('movies',{}))}/{local_eps} qism"
    )
...
def _merge_db_triple(firebase: dict | None, blob: dict | None, local: dict | None) -> dict:
    ...
    """
    firebase = firebase if isinstance(firebase, dict) else {}
    blob     = blob     if isinstance(blob,     dict) else {}
    local    = local    if isinstance(local,    dict) else {}

    # Barcha manbalardagi kinolarni birlashtirish
    fb_movies    = (firebase.get("movies")  or {}) if isinstance(firebase.get("movies"),  dict) else {}
    blob_movies  = (blob.get("movies")      or {}) if isinstance(blob.get("movies"),      dict) else {}
    local_movies = (local.get("movies")     or {}) if isinstance(local.get("movies"),     dict) else {}
    ...
    for code in all_codes:
        candidates = [m for m in [fb_movies.get(code), blob_movies.get(code), local_movies.get(code)] if m]
        if not candidates:
            continue
        if len(candidates) == 1:
        ...
        best = max(candidates, key=lambda m: len((m or {}).get("episodes", []) or []))
    ...
    merged_users = {}
    fb_users    = (firebase.get("users")  or {}) if isinstance(firebase.get("users"),  dict) else {}
    blob_users  = (blob.get("users")      or {}) if isinstance(blob.get("users"),      dict) else {}
    local_users = (local.get("users")     or {}) if isinstance(local.get("users"),     dict) else {}
