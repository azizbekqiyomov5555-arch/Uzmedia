...
# ─── KONFIGURATSIYA ────────────────────────────────────────
BOT_TOKEN  = os.environ.get("BOT_TOKEN")  or "8774359442:AAEsLzhlGSB82O6xPShjkWovv080Yvajb-k"
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("adm_perm|"):
        ...
        try:
            ...
            try:
                await q.answer(
                    f"{'✅ Yoqildi' if new_val else '❌ O\'chirildi'}: {label}",
                    show_alert=False
                )
            except Exception:
                pass
...
async def admin_state_handler(update, context, text: str) -> bool:
    ...
    if state == "block_user_input":
        ...
        if already_blocked:
            ...
            await sm(context.bot, uid,
                f"🔒 <b>{target_name}</b>{uname_str} (<code>{target_uid}</code>) hozir <b>bloklangan</b>.\n\n"
                f"Bu foydalanuvchini blokdan chiqarasizmi?",
                kb)
        else:
            # Bloklash tugmasi
            kb = ikb([[
            ...
            await sm(context.bot, uid,
                f"👤 <b>{target_name}</b>{uname_str} (<code>{target_uid}</code>)\n\n"
                f"Bu foydalanuvchini botdan bloqlaysizmi?\n"
                f"<i>Bloqlangandan so'ng bot bilan muloqot qila olmaydi.</i>",
                kb)
        return True
    ...
    if state == "qism_och_ep":
        ...
        # ── ✅ YANGI: Diapazon formati: "1+10" yoki "1-10" ──
        ...
        if range_match:
            ...
            context.user_data.pop("qism_och_target_uid", None)
            context.user_data.pop("qism_och_code", None)
            already_info = f" ({already_count} ta allaqachon ochiq edi)" if already_count else ""
            try:
                await sm(context.bot, int(target_uid),
                    f"🎉 <b>Admin sizga {start_ep}–{end_ep}-qismlarni ochib berdi!</b>\n\n"
                    f"🎬 <b>{movie.get('title', code)}</b>\n"
                    f"Kino kodini yuboring va tomosha qiling 🍿")
            except Exception as e:
                logger.warning(f"qism_och range notify xato: {e}")
            await sm(context.bot, uid,
                f"✅ <b>{target_name}</b> uchun <b>{code}</b> — "
                f"<b>{start_ep}–{end_ep}-qismlar</b> ({opened_count} ta) ochildi{already_info}!",
                admin_menu_kb(uid))
            return True
