...
def _make_fallback_png(movie_slice, page, total_pages, total_count, start_offset):
    """
    PIL o'rnatilmagan bo'lsa ham ishlaydigan pure-Python PNG generatori.
    Oddiy rangli bloklar bilan chiroyli ro'yxat yasaydi.
    """
    import zlib, struct
    ...
    n     = len(movie_slice)
    H     = HEADER_H + PAD_TOP + n * (CARD_H + GAP) + FOOTER_H
    ...
    # Header
    fill_rect(0, 0, W, HEADER_H, H_BG)
    # Header alt chiziq
    fill_rect(0, HEADER_H-4*SCALE, W, HEADER_H, (255, 200, 50))

    # Footer
    fill_rect(0, H - FOOTER_H, W, H, F_BG)
    fill_rect(0, H - FOOTER_H, W, H - FOOTER_H + 4*SCALE, (255, 200, 50))

    # Kartalar
    for idx in range(n):
        col = ACCENTS[idx % len(ACCENTS)]
        col_light = tuple(min(255, c + 120) for c in col)
        y0 = HEADER_H + PAD_TOP + idx * (CARD_H + GAP)
        y1 = y0 + CARD_H
        # Karta foni (oq)
        fill_rect(PAD_X := 32*SCALE, y0, W - PAD_X, y1, WHITE)
        # Chap rangli chiziq
        fill_rect(PAD_X, y0, PAD_X + 12*SCALE, y1, col)
        # Raqam badji doirasi
        bx = PAD_X + 12*SCALE + 20*SCALE
        by = y0 + CARD_H // 2
        br = 44*SCALE
        fill_rect(bx, by - br, bx + br*2, by + br, col)
...
def generate_movies_image(movie_slice: list, page: int = 1, total_pages: int = 1,
    ...
    # ── PIL bilan to'liq chiroyli rasm ─────────────────────
    if PIL_AVAILABLE:
        try:
            return _generate_movies_image_pil(
                movie_slice, page, total_pages, total_count, start_offset)
        except Exception as e:
            logger.warning(f"PIL rasm xato, fallback: {e}")

    # ── PIL yo'q — pure Python PNG ──────────────────────────
    try:
        return _make_fallback_png(
            movie_slice, page, total_pages, total_count, start_offset)
    except Exception as e:
        logger.error(f"Fallback PNG xato: {e}")
        return None


def _generate_movies_image_pil(movie_slice: list, page: int = 1, total_pages: int = 1,
                                total_count: int = 0, start_offset: int = 0) -> BytesIO | None:
    """PIL bilan to'liq chiroyli kino ro'yxati rasmi"""
...
async def _send_kino_list_page(bot, chat_id: int, page: int = 0, query=None):
    ...
    # ── Har doim surat yasash (PIL bor yoki yo'q) ──────────
    img_buf = None
    try:
    ...
    # Sahifa o'tishda — eski xabarni edit qilamiz
    if query is not None and img_buf is not None:
        try:
            ...
            logger.warning(f"kino_list edit_media xato: {e}")
            try: img_buf.seek(0)
            except Exception: pass

    # Yangi xabar — surat bilan
    if img_buf:
        try: img_buf.seek(0)
        except Exception: pass
    ...
    # Oxirgi fallback — matn (bu holatga deyarli tushmasin)
    lines = []
    for i, (c, m) in enumerate(slice_items, start=start+1):
        ep_c = len(m.get("episodes", []) or [])
        lines.append(f"{i}. <b>{_strip_html(m.get('title', c))}</b> — Kod: <code>{c}</code> | {ep_c} qism")
    text_list = "\n".join(lines)
