...
def generate_movies_image(movie_slice: list, page: int = 1, total_pages: int = 1,
    ...
    BG_DARK      = (8,  11,  24)       # Asosiy fon — koʻk-qora
    BG_CARD      = (16, 21,  44)       # Karta foni — toʻq koʻk
    BG_CARD2     = (13, 18,  38)       # Alternativ karta
    GOLD         = (255, 200, 50)      # Oltin sariq
    GOLD_DARK    = (200, 150, 20)      # Toʻq oltin
    WHITE        = (255, 255, 255)
    GREY_LIGHT   = (180, 185, 210)
    CYAN         = (60,  200, 220)
    GREEN_SOFT   = (50,  210, 130)
    PINK_SOFT    = (255, 100, 140)
    PURPLE_SOFT  = (160, 100, 255)
    ORANGE_SOFT  = (255, 160,  60)
    TEAL_SOFT    = (40,  190, 190)

    ACCENT_COLORS = [
        (80, 140, 255),   # Koʻk
        (50, 210, 130),   # Yashil
        (255, 100, 140),  # Pushti
        (255, 160,  60),  # Toʻq sariq
        (160, 100, 255),  # Binafsha
        (40,  190, 190),  # Moviy-yashil
        (255,  80,  80),  # Qizil
        (80,  200,  90),  # Ochiq yashil
    ]

    S        = 2            # masshtab koeffitsienti (2x = HD)
    IMG_W    = 1080 * S
    PAD_X    = 36   * S
    CARD_H   = 160  * S
    GAP      = 16   * S
    ...
    TOP_PAD  = 18   * S
    RADIUS   = 22   * S
    STRIP_W  = 10   * S    # Karta chap rangli chiziq
    BADGE_R  = 52   * S    # Raqam doirasi radiusi
    ...
    def _tw(draw_obj, text, font):
        try:    bb = draw_obj.textbbox((0, 0), text, font=font); return bb[2] - bb[0], bb[3] - bb[1]
    ...
    # ══ HEADER ═══════════════════════════════════════════════
    # Gradient effekti (qatlam-qatlam to'rtburchaklar)
    ...
    # Oltin chiziq — header pastki chegarasi
    draw.rectangle([(0, HEADER_H - 6*S), (IMG_W, HEADER_H)], fill=GOLD)

    # Kino lenta nuqtalari — header ust
    dot_r = 10 * S
    for xi in range(dot_r * 2, IMG_W - dot_r * 2, dot_r * 4):
        draw.ellipse([xi - dot_r//2, 6*S, xi + dot_r//2, 6*S + dot_r],
                     fill=GOLD_DARK)
    ...
    # Soya
    draw.text((tx0 + 4*S, ty0 + 4*S), title_txt, fill=(0, 0, 0, 120), font=fnt_h1)
    ...
    for idx, (code, movie) in enumerate(movie_slice):
        ...
        col_dim = tuple(max(0, c - 60) for c in col)   # Toʻqroq variant
        ...
        chip_pad = 6 * S
        draw.rounded_rectangle(
            [tx - chip_pad, info_y - chip_pad//2,
             tx + cw + chip_pad, info_y + ch_ + chip_pad//2],
            radius=8*S, fill=col_dim)
        draw.text((tx, info_y), code_txt, fill=WHITE, font=fnt_code)
        after_code = tx + cw + chip_pad + 18*S

        # Qismlar
        ep_txt = f"  {ep_count} qism  "
        ew, eh  = _tw(draw, ep_txt, fnt_sub)
        draw.rounded_rectangle(
            [after_code - 6*S, info_y,
             after_code + ew + 6*S, info_y + eh + 4*S],
            radius=6*S, fill=(30, 45, 80))
        draw.text((after_code, info_y + 2*S), ep_txt, fill=GREEN_SOFT, font=fnt_sub)
        after_ep = after_code + ew + 18*S

        # Ko'rishlar
        views_txt = f"  {views_total} korilgan"
        draw.text((after_ep, info_y + 2*S), views_txt, fill=GREY_LIGHT, font=fnt_sub)

        # ── O'ng tomon bezak nuqtalari ────────────────────
        for di in range(3):
    ...
    if total_pages > 1:
        ...
        f_txt   = f"{start_n}-{end_n} ko'rsatildi  |  Jami {total_count} ta  |  Kino kodini yuboring!"
    else:
        f_txt = f"Jami {total_count} ta kino  |  Kino kodini yuboring! ⬆"
    fw, fh_ = _tw(draw, f_txt, fnt_footer)
    draw.text(((IMG_W - fw) // 2, fy0 + (FOOTER_H - fh_) // 2),
              f_txt, fill=GREY_LIGHT, font=fnt_footer)

    # Lenta nuqtalari — footer
    for xi in range(dot_r * 2, IMG_W - dot_r * 2, dot_r * 4):
        draw.ellipse([xi - dot_r//2, img_h - 6*S - dot_r, xi + dot_r//2, img_h - 6*S],
                     fill=GOLD_DARK)
